...
def run_checks(RECURSIVE, DEPTH, STARTpath, ENDpath):
    ...
    with open(os.devnull, 'wb') as devnull:
        process = subprocess.Popen(shlex.split('voms-proxy-info -exists -valid 0:10'),
                                   stdout=devnull,
    ...
    for FILE in FILES:
        ...
        try:
            if symlinks and os.path.islink(srcname):
                ...
                if srel == "./":
                    srel = ""
                copy_command, start_location, end_location = init_commands(STARTsite,
                                                                           ENDsite,
                                                                           PROTOCOL,
                                                                           RECURSIVE,
                                                                           VERBOSE,
...
def main(START, STARTpath, START_USER, END, ENDpath, END_USER, PROTOCOL, SAMPLE, DIFF, DELETE,
    ...
    if STARTsite.alias!='local': GetSiteInfo.get_site_info(site=STARTsite,debug=args.debug,fast=True,quiet=True)
    ENDsite = Location(END,END_USER,ENDpath+"/" if END!='local' else ENDpath)
    if ENDsite.alias!='local': GetSiteInfo.get_site_info(site=ENDsite,debug=args.debug,fast=True,quiet=True)
...
And those are the options available. Deal with it.""")
    ...
    parser.add_argument("STARTpath",
                        help="The location of the files within the users store area or the " \
                             "absolute path if the STARTserver is \'local\'")
